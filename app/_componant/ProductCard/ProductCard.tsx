    ...
    return (
        <div className="group relative bg-white rounded-xl border border-gray-100 p-4 pt-10 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 h-full flex flex-col">
            ...
            <div className="absolute top-4 right-4 flex flex-col gap-2.5 z-20">
                ...
                {/* زر القلب - مركز في النص */}
                <div className="flex items-center justify-center w-9 h-9">
                    <WishlistBtn
                        productId={product._id}
                        onlyIcon={true}
                        classs="w-9 h-9 rounded-full border shadow-sm flex items-center justify-center"
                    />
                </div>

                {/* زر التحديث */}
                <button className="p-2.5 bg-white rounded-full shadow-md hover:bg-green-50 text-gray-500 hover:text-green-600 transition-colors flex items-center justify-center">
                    <RefreshCw size={15} />
                </button>

                {/* زر العين */}
                <Link href={`/productdetails/${product._id}`}>
                    <button className="p-2.5 bg-white rounded-full shadow-md hover:bg-blue-50 text-gray-500 hover:text-blue-600 transition-colors flex items-center justify-center">
                        <Eye size={15} />
                    </button>
                </Link>
